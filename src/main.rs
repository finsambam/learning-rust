// mod ch3_1_variables_and_mutability;
// mod ch3_2_1_integer_scalar_data_types;
// mod ch3_2_2_floating_point_scalar_data_types;
// mod ch3_2_3_boolean_and_character_scalar_data_types;
// mod ch3_2_4_compound_data_types;
// mod ch3_3_1_basic_of_function;
// mod ch3_3_2_statement_vs_expression;
// mod ch3_5_1_control_flow_if;
// mod ch3_5_2_control_flow_loop;
// mod ch4_1_0_ownership_pre_test;
// mod ch4_1_1_what_is_ownership;
// mod ch4_2_0_reference_and_borrowing;
mod ch4_2_1_lifetime_anotation;

fn main() {
    println!("Rust Learning Project");
    
    // ch3_1_variables_and_mutability::run();
    // ch3_2_1_integer_scalar_data_types::run();
    // ch3_2_2_floating_point_scalar_data_types::run();
    // ch3_2_3_boolean_and_character_scalar_data_types::run();
    // ch3_2_4_compound_data_types::run();
    // ch3_3_1_basic_of_function::run();
    // ch3_3_2_statement_vs_expression::run();
    // ch3_5_1_control_flow_if::run();
    // ch3_5_2_control_flow_loop::run();
    // ch4_1_0_ownership_pre_test::run();
    // ch4_1_1_what_is_ownership::run();'
    // ch4_2_0_reference_and_borrowing::run();
    ch4_2_1_lifetime_anotation::run();
}
